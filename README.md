...
│ acquisition_method VARCHAR(50)│      │ notes       TEXT             │
│ purchase_price  FLOAT        │       │ created_at  DATETIME         │
│ status      VARCHAR(20)      │       │ updated_at  DATETIME         │
│ batch_no    VARCHAR(50)      │       │ deleted_at  DATETIME (NULL)  │
...
关系: items 1 ──── N exchanges
软删除: deleted_at 非空表示已删除，查询自动过滤
```
...
| POST | `/api/exchanges` | 新增置换记录（需关联 item_id） |
| PUT | `/api/exchanges/{id}` | 更新置换记录 |
...
- **软删除**: 删除操作仅设置 `deleted_at` 时间戳，数据保留可恢复
